...
FILE * fp = fopen("mydata1.csv", "w");  // opens file to write ("w"), using the FILE pointer "fp"
...
fprintf(fp,"Day,Inventory in beginning,Random Number 1, Random Number 2, Demand,Ending Inventory,Quantity Ordered,Total Ordering Cost, Holding Cost,Shortage Cost\n");

    for(k=0;k<50;k++)		// Output is generated
    {
        for(t=0;t<10;t++)
        {
            fprintf(fp," %lf, ",arr[k][t]);
        }
